...
def gamma_prop(v, j, gamma, theta, coord, neigh):
    part = np.sum([model.w(coord, v, k)*(1-2*gamma[k, j]) for k in neigh[v]])
    return 1/(1 + np.exp(theta[j]*part))
...
# ratio gamma
def ratio_gamma(v, j, gamma, gamma_star, cov_m_inv, data, tp, design_m): # Hastings ratio for updating gamma
...
# update gamma
def update_gamma(v, j, gamma_cur, theta_cur, coord, neigh, cov_m_inv, data, tp, design_m):
    ...
    while 1:
        ...
        for v in range(N):
            for j in range(p - 2): # first two colums are one's
                gamma_cur = update_gamma(v, j+2, gamma_cur, theta_cur, coord, neigh, cov_m_inv, data, tp, design_m) # update gamma
