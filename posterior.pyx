...
# update gamma
cpdef double update_gamma(int v, int j, np.ndarray[double, ndim = 2] gamma_cur, double theta_cur, np.ndarray[int, ndim = 1] neigh, np.ndarray[double, ndim = 2] cov_m_inv, np.ndarray[double, ndim = 1] data, double tp, np.ndarray[double, ndim = 2] design_m):
...
cpdef int mcmc_update(dict neigh, np.ndarray[double, ndim = 3] cov_m_inv, np.ndarray[double, ndim = 2] data, double tp, np.ndarray[double, ndim = 2] design_m, int p, int N, bytes dirname):
    ...
    cdef int i, v, j
    cdef np.ndarray[int, ndim = 1] b = (2**9)*np.ones(2, dtype = int)   # start to check at 2**(2*9)
    cdef double a
...
#             cond_gamma = (mcse_gamma[:, 0]*1.645+1.0/n - 0.1*mcse_gamma[:, 1])
            ...
            cond_theta = (mcse_theta*1.645 - 0.1*np.sqrt(theta_std[:, 2]))
            cond_gamma = (mcse_gamma*1.645 - 0.1*np.sqrt(gamma_std[:, 2]))
